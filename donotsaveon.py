...
class Family(object):
    ...
    def DFS_for_family(Family, start, path= []):
        path = path + [start]
        if Family.names_to_nodes[start].get_parent() == None:
            return len(path) - 1
        else:
            return DFS_for_family(Family, Family.names_to_nodes[start].get_parent().__str__() , path)

    def cousin(self, a, b):
        if self.is_child(a, b) or self.is_child(b, a):
            return (-1, 0)
        if f.names_to_nodes[a] == f.names_to_nodes[b]:
            return (-1, 0)
        aa = DFS_for_family(self, a)
        bb = DFS_for_family(self, b)
        removal_number = abs(aa - bb)
        if bb > aa:
            cousin_number = 1 - aa
            return (cousin_number, removal_number)
        elif aa == bb:
            return (1, 0)
        else: 
            cousin_number = 1 - bb
...
'''
def DFS_for_family(Family, start, path= []):
        print('Starting another recursive loop')
        print(type(path))
        path = path + [start]
        if Family.names_to_nodes[start].get_parent() == None:
            return len(path) - 1
